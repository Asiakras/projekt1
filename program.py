...
class Transformacje:
    ...
    def xyz2plh(self, X, Y, Z, output = 'dec_degree'):
        ...
        lon = np.arctan2(Y, X)
        p = np.sqrt(X**2 + Y**2)
        lat = np.arctan(Z / (p*(1-self.ecc2)))
        while True:
            N = self.a/np.sqrt(1-self.ecc2*np.sin(lat)**2)
            h = p/np.cos(lat) - N
            fs = lat
            lat = np.arctan(Z/(p*(1-(self.ecc2*(N/(N+h))))))
...
if __name__ == "__main__":
    ...
    if ellipsoid_model == 'mars' and (transformation_function == 'pl2000' or transformation_function == 'pl1992'):
        print("Model not supported with this transformation function. Choose a different model.")
        sys.exit(1)  
    ...
    if transformation_function == 'xyz2plh':
        ...
        with open(input_file, 'r') as f:
            ...
            for line in lines:
                ...
                x0, y0, z0 = [float(coord) for coord in sys.argv[6:9]]
